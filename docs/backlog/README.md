...
| 0020 | Heal phase detects infra-flake CI failures and reruns instead of code-fixing | P1 | shipped | safety |
...
| 0029 | fleet provenance <pr> reconstructs the prompts SHA, lessons, and heal patterns behind a shipped PR | P1 | shipped | governance |
...
| 0059 | fleet stuck flags every PR sitting on a non-actionable cause so the operator unblocks it in one glance | P1 | shipped | observability |
| 0060 | fleet flaky surfaces CI checks that have failed-then-passed without a code change so the kit stops mis-attributing infra noise to ticket regressions | P1 | shipped | observability |
| 0061 | fleet invoice <slug> emits a monthly billing-style ROI receipt suitable for forwarding to a CFO or future-self | P1 | shipped | governance |
| 0062 | fleet why <slug> composes the one-paragraph argument for keeping the kit installed against the slug's own telemetry | P2 | shipped | docs |
| 0063 | fleet migrate --export / --import packages an operator's whole fleet into one tarball so a fresh MacBook is restored in 60 seconds | P1 | in-progress | engine |
| 0064 | fleet catchup composes a re-orientation briefing after the operator's been away >72h so returning to a 4-slug fleet stops feeling like triage | P1 | groomed | observability |
| 0065 | fleet lessons-import codifies a third-party lesson-pack contract so a peer's hard-won LESSONS can pre-load a fresh fleet without copy-paste | P2 | groomed | engine |
