...
> **Verify the install after.** Once `install.sh` returns, `bin/fleet onboarding-check ~/code/your-repo` walks 11 read-only checks against the freshly-installed project (manifest parses, AGENTS.md section present, backlog index present, `~/.local/share/agent-fleet/lib` copied, launchd plists installed + labels enabled, `$CACHE_DIR/events.jsonl` writable, `gh` authenticated, `claude` on PATH, prompts SHA pins to the installed copy) AND runs a synthetic ship cycle that emits the four core event types (`run_started`, `pr_opened`, `lesson_draft_emitted`, `run_completed`) with `phase=onboarding-check` — NO real PR, NO claude spawn, NO heal-counter advance. Prints a GREEN/YELLOW/RED table plus a NEXT block of copy-pasteable next steps. The acquisition trio is now end-to-end: `kickstart --demo` (see the loop) → `preflight` (what would install do) → `install.sh` → `onboarding-check` (is it wired). See [ticket 0041](docs/backlog/0041-fleet-onboarding-check-post-install-verifier.md).
...
~/code/agent-fleet/bin/fleet streak                                            # current + longest continuous green-day run per project (ticket 0042 — --since Nd|YYYY-MM-DD default 90d; --slug/--json; pure reader)
```
